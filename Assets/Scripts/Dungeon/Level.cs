/* NOTES:
    CreateRooms() needs to be able to double back if it makes bad rolls (if all rooms roll 0, it ends prematurely)
        ALTERNATIVE: If a room is the last room on the stack, and the # of rooms hasn't been achived, they MUST roll at least 1 room
    Also apparently a small chance for ArgumentoutofRangeException, don't know where
...
public class Level
{
    ...
    // Number of rooms in this level
    private int numRooms;
    // Graph / Point representation of level rooms
    private Dictionary<Point, PHRoom> level;
    ...
    public Level()
    {
        ...
        level = new Dictionary<Point, PHRoom>();
    ...
    public void PrintLevel()
    {
        ...
        int i = 0;
        foreach (KeyValuePair<Point, PHRoom> room in level)
        {
            i++;
            Debug.Log("Room " + i + " @ " + room.Key.ToString() + "\n" + "Doors " + room.Value.GetDoorString());
        }
        if (numRooms != level.Count)
        {
            Debug.Log("Number of rooms [" + numRooms + "] does not match the actual numbers of rooms [" + level.Count + "]");
        }
    }

    public Dictionary<Point, PHRoom> GetLevel()
    {
        return this.level;
    }
    ...
    public void CreateRooms()
    {
        ...
        level.Add(rootRoom.Key, rootRoom.Value);
        countRooms++;
        ...
        while (addRooms > 0)
        {
            ...
            // Add room to level
            level.Add(addRoom.Key, addRoom.Value);
        ...
        while (countRooms < numRooms)
        {
            while (idx < roomQ.Count)
            {
                ...
                foreach (KeyValuePair<Point, int> dir in DIRECTIONS)
                {
                    ...
                    if (level.ContainsKey(curRoom.Key + dir.Key))
                    {
        ...
        foreach (KeyValuePair<Point, PHRoom> room in roomQ)
        {
            ...
            foreach (KeyValuePair<Point, int> dir in DIRECTIONS)
            {
                if (level.ContainsKey(room.Key + dir.Key))
                {
                    numNeighbors++;
                }
            }
    ...
    */
    public void CreateDoors()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (DIRECTIONS.ContainsKey(key))
            {
                ...
                level[rootRoom.Key].doors[DIRECTIONS[key]] = DOOR;
                // Create the door in the checking room
                if (DIRECTIONS[key] < 2)
                {
                    level[key].doors[DIRECTIONS[key] + 2] = DOOR;
                }
                else
                {
                    level[key].doors[DIRECTIONS[key] - 2] = DOOR;
    ...
    private bool FindRoot(Point room)
    {
        ...
        do
        {
            ...
            {
                PHRoom curValue;
                if (!level.TryGetValue(curRoom, out curValue))
                {
                ...
                foreach (KeyValuePair<Point, int> dir in DIRECTIONS)
                {
                    // IF it is a valid neighbor (is connected via a door, is in the level, has NOT been visited, is NOT in the queue)
                    if (curValue.doors[dir.Value] == DOOR &&
                        level.ContainsKey(curRoom + dir.Key) &&
    ...
    private int AddDoors(Point roomKey, List<Point> directions)
    {
        ...
        if (level[roomKey].GetNumDoors() == 0)
        {
    ...
    private int AddDoors(Point roomKey, List<Point> directions, int numAddDoors)
    {
        ...
        while (numAddDoors > 0 && directions.Count > 0)
        {
            ...
            level[roomKey].doors[DIRECTIONS[chosenDir]] = DOOR;
            // Switch between NE EW and SN WE
            if (DIRECTIONS[chosenDir] < 2)
            {
                level[roomKey + chosenDir].doors[DIRECTIONS[chosenDir] + 2] = DOOR;
            }
            else
            {
                level[roomKey + chosenDir].doors[DIRECTIONS[chosenDir] - 2] = DOOR;
    ...
    private List<Point> GetDirections(KeyValuePair<Point, PHRoom> room)
    {
        ...
        foreach (KeyValuePair<Point, int> dir in DIRECTIONS)
        {
            ...
            if (room.Value.doors[dir.Value] == NO_DOOR &&
                level.ContainsKey(room.Key + dir.Key))
            {

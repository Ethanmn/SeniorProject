...
public class DamageBuff : Buff
{
    ...
    public override void OnBegin(Transform character)
    {
        ...
        ico.GetComponent<Image>().sprite = Resources.Load<Sprite>("Sprites/UI/Buffs/PAtk");
    ...
    public override void OnEnd()
    {
        ...
        if (damageTrack == damageAmount)
        {
            if (chr.CompareTag("Mob"))
            {
                chr.GetComponent<MobStats>().BonusDamage -= damageAmount;
            }
            else if (chr.CompareTag("Hero"))
            {
                chr.GetComponent<HeroStats>().BonusDamage -= damageAmount;
            }
            damageTrack = 0;
        }

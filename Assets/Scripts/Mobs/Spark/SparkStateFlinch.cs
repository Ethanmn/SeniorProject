...
public class SparkStateFlinch : I_MobFlinchState {

    float timer;
	Vector2 vel;

	public SparkStateFlinch(Vector2 vel)
	{
		this.vel = vel;
	}
	...
	{
		mob.GetComponent<SpriteRenderer>().sprite = Resources.LoadAll<Sprite>("Sprites/Mobs/SparkPH")[2];
	...
	I_ActorState I_ActorState.Update(Transform mob, float dt)
	{
		...
		if (timer <= 0)
		{
            ...
            if (stats.Health <= 0)
            {
                ...
                return new SparkStateDeath();
            }

            return new SparkStateAlert();
		}
